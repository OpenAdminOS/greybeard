...
When the server is unavailable, shared memory reports an error. Hooks return empty context within a 1.5-second processing deadline and do not block the host. There is no silent fallback to local records, offline memory cache, or replay queue. An interrupted mutation may have committed: inspect the current records before repeating it. Request IDs deduplicate transport replays for 24 hours; requests older than that are rejected. The server retains replay records for two days. Forgetting records discards stored mutation response snapshots while retaining replay tombstones, so old retries cannot recreate forgotten content.
