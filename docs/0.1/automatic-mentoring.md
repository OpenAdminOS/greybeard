...
- Real Claude Code model authentication failed: OAuth session expired and could not be refreshed. Actual Claude Code hook delivery and preference proposals pass against an isolated synthetic Anthropic-compatible endpoint. Refresh that account's local Claude authentication before repeating real model reasoning checks. Windows Desktop Code subsequently passed real preference capture, exact companion confirmation and reuse in a later model conversation. See the Windows desktop evidence.
- Gemini CLI 0.59.0 loaded the configuration but exited with code 41 because no authentication method is configured in the isolated environment. Actual Gemini hook execution and outgoing context delivery now pass against a local synthetic provider, including automatic preference proposals; see the native transport evidence below. Complete an authorized Gemini login before real model reasoning verification.
- Copilot CLI 1.0.83 reached the account policy check but was denied access; it also reported third-party MCP disabled by organization policy. Enable applicable Copilot CLI access at the account/organization level before repeating real model verification. Actual Copilot CLI hook execution and outgoing context delivery now pass in its documented offline local-provider mode, including automatic preference proposals. This does not establish access under the blocked account policy or verify model reasoning.
- Cursor 3.20.17 on Windows now has observed automatic prompt proposals, native companion notifications and post-tool context containing the confirmed lesson. The live test exposed a UTF-8 BOM parsing defect that is fixed and regression-covered. Mac editor delivery remains unverified. Its prompt hook does not support advisory model context. Prompt-time advice therefore needs the running Greybeard companion and permitted native notifications.
...
The [native transport evidence](../../evaluations/automatic-0.1/transport/README.md) records actual Claude Code, Gemini and Copilot CLI runs with a synthetic local response server. It exposed and verifies the fix for Gemini's prepended session-context blocks, which previously displaced the user task from the short recall query. All three hosts retain the user prompt, receive the confirmed lesson and create a single reviewed preference candidate.
...
[Windows desktop evidence and limits](../../evaluations/automatic-0.1/windows-desktop/README.md) distinguish actual host events from model reasoning and document the encoding regression.
