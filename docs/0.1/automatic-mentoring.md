...
- Real Claude Code model authentication failed: OAuth session expired and could not be refreshed. Actual Claude Code hook delivery and preference proposals pass against an isolated synthetic Anthropic-compatible endpoint. Refresh that account's local Claude authentication before repeating real model reasoning checks. Desktop Code shares the documented local hooks but was not directly driven from this Linux machine.
