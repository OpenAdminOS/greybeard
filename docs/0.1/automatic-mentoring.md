...
The [native transport evidence](../../evaluations/automatic-0.1/transport/README.md) records actual Gemini and Copilot CLI runs with a synthetic local response server. It exposed and verifies the fix for Gemini's prepended session-context blocks, which previously displaced the user task from the short recall query. Both hosts retain the user prompt, receive the confirmed lesson and create a single reviewed preference candidate.
