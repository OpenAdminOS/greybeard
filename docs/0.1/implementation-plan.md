...
- Optional tenant connection uses a customer-owned app registration and app-only authentication. Collect a profile name, Tenant ID, Client ID, and a customer-provisioned certificate credential. No ongoing end-user browser sign-in. An authorized tenant administrator still configures the registration and grants application permissions in Entra.
