...
- Set an end-to-end remote hook deadline of at most 1.5 seconds, within the existing five-second host hook limits. Bound local preprocessing too. Do not retry hooks within that budget. A timeout returns the existing empty host response and never blocks the user's command.
