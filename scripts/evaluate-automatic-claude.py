...
OUT=ROOT/'evaluations/automatic-0.1/claude-installed'
...
for id,fixture,prompt in CASES:
 ...
 with tempfile.TemporaryDirectory(prefix='greybeard-evaluation-',dir='/tmp') as tmp:
  ...
  subprocess.run(['node',str(ROOT/'scripts/evaluate-installed-host.mjs'),str(home),str(app),'claude','automatic'],check=True,capture_output=True,cwd=ROOT)
  ...
  result=dict(automatic=ledger,id=id,fixture=fixture,prompt=prompt,exitCode=code,toolCalls=calls,response=response,resultEvents=[e for e in events if e.get('type')=='result'])
