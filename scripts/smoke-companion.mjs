...
try {
  ...
  await expect(window.locator('#map-select option')).toHaveCount(67);
  const globeBefore = await window.locator('#memory-globe').innerHTML();
  await window.locator('#memory-globe').focus();
  await window.keyboard.press('ArrowRight');
  await expect.poll(() => window.locator('#memory-globe').innerHTML()).not.toBe(globeBefore);
  ...
  await expect(window.locator('#map-select option')).toHaveCount(0);
  await expect(window.locator('#map-empty')).toBeVisible();
  ...
  await window.getByRole('button', { name: 'Memory map', exact: true }).click();
  await expect(window.locator('#map-select option')).toHaveCount(66);
  await expect(window.locator('#map-select')).not.toContainText('72 hours');
  await expect(window.locator('#map-detail')).not.toContainText('Corrected by');
  await window.getByRole('button', { name: 'Your memory', exact: true }).click();
